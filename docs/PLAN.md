...
**やること:**
- [ ] termchat v1.3.1 を fork してリポジトリ作成
- [ ] `Cargo.toml` の `name`, `description`, `homepage`, `repository` を更新
- [ ] `COMMAND_PREFIX` を `"?"` → `"/"` に変更
- [ ] `Config::from_config_file()` のパスを `"termchat"` → `"triadchat"` に変更
- [ ] video stream feature (`stream-video`) を Cargo.toml から除外
- [ ] `cargo build` と既存 `cargo test` が通ることを確認
...
- [ ] `cargo test` が通る
- [ ] 起動して既存チャット機能が動く (コマンドプレフィックスが `/` になっている)
...
- [ ] `Cargo.toml` に依存追加:
  ...
  serde_json = "1"
  ```
- [ ] `Signal` に `AiResponse(String)` (暫定型) と `SkillDone` を追加
- [ ] `Application` に `tokio::runtime::Runtime` フィールドを追加
- [ ] `Application::new()` で `Runtime::new()` を生成、`handle()` を保持
- [ ] **`#[tokio::main]` は使わない** (message-io スレッドとの競合回避)
- [ ] ダミーの `handle.spawn(async { ... signal ... })` でループ内受信を確認するテストを書く
...
- [ ] `cargo build` が通る
- [ ] 既存チャット機能が壊れていない
- [ ] tokio spawn したタスクから `Signal::AiResponse` を送り、アプリループで受信できる (統合テスト)
...
- [ ] `Application::new()` で `SidecarAdapter::new()` を呼び、失敗したら警告表示 + `provider = disabled` にフォール
- [ ] `tests/sidecar_mock.rs`: `claude` を `echo` に差し替えた mock で `ask()` が `Ok` を返すこと
- [ ] タイムアウトテスト: `sleep 31` を実行して `Err` が返ること

**リスク:** `claude -p` の出力形式変化  
**対策:** stdout を raw String で返し、パースは呼び出し元に委譲

**受け入れ基準:**
- [ ] `claude` コマンドがない環境で起動時にわかりやすいエラーが出る
- [ ] 30秒でタイムアウトして `Err` が返る (テスト)
- [ ] mock で `ask()` が正常に `Ok(String)` を返す (テスト)
...
**やること:**
- [ ] `LanguageConfig` を `src/config.rs` に追加:
  ```rust
  ...
  ```
- [ ] `Default` 実装: `$LANG` 環境変数から自動判定、不明なら `"ja"`
- [ ] `Config` に `pub language: LanguageConfig` を追加
- [ ] `config.toml` のデフォルト生成に `[language]` セクションを追加
- [ ] `src/ai/prompt.rs` に `lang_instruction(lang: &str) -> &'static str` を追加:
  - `"ja"` → `"必ず日本語で出力してください。"`
  - `"en"` → `"Respond in English."`
  - `"zh"` → `"请用中文回答。"`
  - `"ko"` → `"한국어로 답변해 주세요。"`
  - その他 → `"Respond in English."` (フォールバック)
- [ ] `src/ui/messages.rs` に UI システムメッセージを定義:
  ```rust
  pub struct Messages {
  ...
  ```
- [ ] 既存のハードコードされた英語文字列を `messages()` に置き換え
...
**受け入れ基準:**
- [ ] `config.toml` に `[language]` セクションが生成される
- [ ] `ai_output = "en"` に設定すると AI が英語で応答する
- [ ] `ui = "en"` に設定すると接続通知・エラーが英語になる
- [ ] `$LANG=ja_JP.UTF-8` の環境でデフォルトが `"ja"` になる
- [ ] `$LANG=en_US.UTF-8` の環境でデフォルトが `"en"` になる
- [ ] 未知の言語コードを設定してもパニックしない (フォールバック)
...
- [ ] `AiPayload`, `StructuredOutput`, `TodoItem` を `src/message.rs` に定義
- [ ] テスト用会話フィクスチャを `tests/fixtures/` に作成 (最低5パターン)
- [ ] `tests/prompt_quality.rs` で期待出力との一致を検証
  - 担当者抽出の精度
  - 決定事項の正確な抽出
  - 雑談では SKIP が返ること
...
**受け入れ基準:**
- [ ] 5パターンのフィクスチャで TODO/決定事項が正確に抽出される
- [ ] パース失敗 (想定外フォーマット) でパニックしない
- [ ] `truncate_transcript()` が 100 行超を正しく切り詰める
...
**やること:**
- [ ] `AiState`, `AiMode` を `src/state.rs` に追加
- [ ] `AppState` に以下を追加:
  - `ai_state: AiState`
  - `ai_mode: AiMode`
  - `ai_thinking: bool`
  ...
  - `human_streak: usize`
- [ ] `src/ai/trigger.rs` — `should_intervene()` を実装
  - `ai_thinking == true` → false
  - cooldown (30秒) チェック
  - human streak (3連続) チェック
  - mode 別判定
- [ ] `src/ai/classifier.rs` — `MessageClass` 分類を実装
- [ ] `Application::process_terminal_event()` の Enter 処理に介入判定を追加
- [ ] `Application::process_ai_response()` を実装:
  - `state.ai_thinking = false`
  - `state.ai_state = Idle`
  - `state.add_message(AiChatMessage)`
- [ ] AI が thinking 中はチャットパネルのタイトルに `[ops-ai: thinking...]` を表示
- [ ] `tests/trigger_test.rs` — 全条件のユニットテスト

**リスク:** `ai_thinking` のリーク (spawn panic 時)  
**対策:** `AbortHandle` を `AppState` に保持、新しい spawn 前に前の handle を abort

**受け入れ基準:**
- [ ] thinking 中にヘッダー表示が変わる
- [ ] 二重呼び出しが発生しない (テスト)
- [ ] `should_intervene()` の全条件がテストを通る
...
**やること:**
- [ ] `src/commands/summary_cmd.rs` を実装:
...
- [ ] AI メッセージを `✦` マーク + 別色で描画
- [ ] `/help` に新コマンドを追加
- [ ] Phase 0 受け入れ基準全項目を手動確認

**受け入れ基準 (Phase 0 完了ライン):**
- [ ] `/summary` で要約が出る
- [ ] `/todos` で担当者付き TODO が出る
- [ ] `/decisions` で決定事項が出る
- [ ] AI の自動介入が clerk mode で動く
- [ ] 30秒タイムアウトで `[ops-ai: failed]` 表示
- [ ] `/ai mode listener` で AI が黙る
- [ ] `claude` なし環境で起動時エラーが出る
...
**やること:**
- [ ] `NetMessage` に新 variant を **末尾に追加** (bincode 互換)
  ```
  PeerInfo(PeerInfo)
  ...
  ```
- [ ] `PeerInfo`, `SkillResultPayload` 型を定義
- [ ] `process_network_message()` に新 variant のマッチアームを追加 (暫定: ログ出力のみ)
- [ ] **後方互換テスト:** 旧 `HelloUser` だけを送るノードと接続できること
...
**受け入れ基準:**
- [ ] `cargo build` が通る
- [ ] 既存の HelloLan/HelloUser/UserMessage フローが壊れていない
- [ ] `NetMessage::PeerInfo` をエンコード→デコードできる (ユニットテスト)
...
**やること:**
- [ ] `Room`, `Member`, `MemberKind`, `RoomEngine` を実装
- [ ] `AppState` に `peers: HashMap<Endpoint, PeerInfo>`, `rooms: Vec<Room>`, `active_room_id` を追加
- [ ] `HelloUser` 受信後に `NetMessage::PeerInfo` を自動送信するフローを追加
- [ ] `RoomCreate` 送信 → `RoomJoin` 受信 → ルーム確立
- [ ] `/room create @user1 [--ai clerk]` コマンドを実装
  - AI participant を自動アタッチ (v1: 起動ノード上で動作)
- [ ] `/room list`, `/room switch`, `/peers` コマンドを実装
- [ ] `tests/network_integration.rs` — 2プロセス起動して peer discovery が 3秒以内に成立することを確認
...
**やること:**
- [ ] `SkillMeta`, `SkillScope`, `InvokeMode`, `RiskLevel` を定義
- [ ] `SkillRegistry::scan(workspace)` を実装:
...
- [ ] `/skills` コマンド — 一覧を表形式で表示 (name / risk / invoke_mode / description)
- [ ] テスト用フィクスチャの SKILL.md を作成
- [ ] `tests/skill_registry.rs` — スキャン・パース・キャッシュのユニットテスト

**受け入れ基準:**
- [ ] `.claude/skills/` がない場合でも起動・動作する
- [ ] `/skills` でスキャン結果が表示される
- [ ] frontmatter パースエラーの SKILL.md がある場合、他のスキルは正常に読める
...
**やること:**
- [ ] `SkillExecutor::run(meta, args, handle, sidecar)` を実装:
  - skill タイムアウト: 60秒
  - `Signal::SkillDone(SkillResultPayload)` を送信
- [ ] `/skill <name> [args]` コマンド:
...
- [ ] Enter で `pending_confirmation` がある場合、`y/n` キー処理を追加
- [ ] `process_skill_done()` — 結果をチャットに投稿、`state.ai_state = Idle`
- [ ] `/run <id>` — AI が提案したスキルを番号で実行
- [ ] `/cancel` — `abort_handle.abort()` で実行中タスクを中止
- [ ] `tests/skill_executor.rs` — confirm → 実行 → SkillDone の状態遷移テスト

**受け入れ基準:**
- [ ] `/skill review-auth` で承認ダイアログが出る
- [ ] `y` で非同期実行が開始、実行中は `[acting]` 表示になる
- [ ] 実行結果がチャットに投稿される
- [ ] `risk:medium` スキルは確認なしに実行されない
- [ ] `/cancel` で実行中タスクが止まる
...
**やること:**
- [ ] termchat の `src/commands/send_file.rs` を移植
  - `?send` → `/send` に変更 (COMMAND_PREFIX 変更で自動対応済みのはず)
- [ ] ファイル受信時のシステムメッセージをそのまま維持
- [ ] `/send` のヘルプテキストを更新

**受け入れ基準:**
- [ ] `/send ./error.log` でファイルを送信できる
- [ ] 相手側に受信完了メッセージが出る
...
**やること:**
- [ ] `TranscriptWriter` を実装:
  - 保存先: `~/.local/share/triadchat/transcripts/<room_id>.jsonl`
  - `create_dir_all` でディレクトリを自動作成
  - append mode、`Drop` で flush
- [ ] `TranscriptEntry` を定義 (message.rs の型を再利用)
- [ ] `Application` でメッセージ追加のたびに `transcript.append()` を呼ぶ
  - write 失敗は warn ログのみ (チャットを止めない)
- [ ] `tests/transcript.rs` — JSONL ラウンドトリップテスト

**受け入れ基準 (Phase 1 完了ライン):**
- [ ] セッション後に `~/.local/share/triadchat/transcripts/<room_id>.jsonl` が存在する
- [ ] JSONL が正しいフォーマットで書かれている
- [ ] Transcript write 失敗でチャットが止まらない (テスト)
- [ ] Phase 1 受け入れ基準 全項目が通る
...
**やること:**
- [ ] `AvatarPlugin` トレイトを定義
- [ ] `AvatarPluginVTable` (`#[repr(C)]` FFI vtable) を定義
  - vtable にバージョン番号フィールドを含める
- [ ] `builtin.rs` に組み込みプリセットを実装:
  - `human_default` (3サイズ)
  - `ai_default` (state 別 × 3サイズ)
  - `robot_guardian` (state 別 × 3サイズ)
- [ ] `AvatarSize::Compact` を 80列未満で自動選択するロジック
- [ ] `tests/avatar_builtin.rs` — 全プリセット × 全 state × 全サイズの組み合わせテスト

**受け入れ基準:**
- [ ] 全プリセットが全 state × サイズでパニックせず文字列を返す
- [ ] `AvatarPlugin` トレイトを外部クレートで実装できるインターフェースになっている
...
- [ ] `src/ui/mod.rs` を3ペインに変更:
  ...
  ```
- [ ] `peers_panel::draw()` — peer 一覧 + avatar (compact) + presence
  - presence: `online` / `idle` / `busy` / `offline`
- [ ] `status_panel::draw()` — AI avatar (normal) + mode + state + TODO 直近5件 + スキル提案
- [ ] ターミナル幅 < 80列のとき、peers / status パネルを非表示にしてフォールバック
- [ ] `tests/ui_layout.rs` — `ui-test` feature を使ったサイズ計算テスト

**受け入れ基準:**
- [ ] 3ペインが表示される
- [ ] AI state 変化が peers_panel と status_panel 両方に反映される
- [ ] 幅不足時に2ペインにフォールバックする
...
- [ ] `tests/avatar_plugin.rs` — テスト用の最小 dylib を作成し、ロード + render が動くことを確認

**リスク:** FFI ABI の安定性  
**対策:** `#[repr(C)]` + `extern "C"` + vtable バージョンフィールドで管理

**受け入れ基準:**
- [ ] `~/.config/triadchat/avatars/` に dylib を置くとロードされる
- [ ] vtable バージョン不一致の dylib はスキップされる
- [ ] builtin プリセットは常にフォールバックとして機能する
...
**やること:**
- [ ] `/avatar set <target> <preset>` — 対象の avatar preset を変更
- [ ] `/avatar preview` — 現在のアバターを全サイズで表示
- [ ] `/avatar mode <compact|normal|expressive>` — グローバルサイズ変更
- [ ] `/avatar list` — `AvatarManager::list_all_presets()` の結果を表示
- [ ] `config.toml` の `user.avatar` を起動時に反映

**受け入れ基準 (Phase 2 完了ライン):**
- [ ] `/avatar set @ops-ai robot_guardian` でアバターが変わる
- [ ] `/avatar list` にプラグインのプリセットが出る
- [ ] ASCII avatar が AI state に応じて変化する
- [ ] Phase 2 受け入れ基準 全項目が通る
...
|----|----------|----------|------|-----------|
| PR-01 | fork + rename + prefix 変更 | 0 | — | `📋 TODO` |
| PR-02 | tokio Runtime + Signal 拡張 | 0 | PR-01 | `📋 TODO` |
| PR-03 | SidecarAdapter | 0 | PR-02 | `📋 TODO` |
| PR-04a | 言語設定 (LanguageConfig + UI i18n) | 0 | PR-03 | `📋 TODO` |
| PR-04 | prompt.rs + パーサー ★ | 0 | PR-04a | `📋 TODO` |
| PR-05 | AiMediator + 介入トリガー | 0 | PR-04 | `📋 TODO` |
| PR-06 | /summary /todos /decisions | 0 | PR-05 | `📋 TODO` |
| PR-07 | NetMessage 拡張 + PeerInfo | 1 | PR-02 | `📋 TODO` |
| PR-08 | Room エンジン + LAN discovery | 1 | PR-07 | `📋 TODO` |
| PR-09 | SkillRegistry | 1 | PR-08 | `📋 TODO` |
| PR-10 | SkillExecutor + 承認 UI | 1 | PR-09 | `📋 TODO` |
| PR-11 | /send 移植 | 1 | PR-10 | `📋 TODO` |
| PR-12 | Transcript JSONL | 1 | PR-10 | `📋 TODO` |
| PR-13 | AvatarPlugin trait + builtin | 2 | PR-06 | `📋 TODO` |
| PR-14 | 3ペイン TUI | 2 | PR-13 | `📋 TODO` |
| PR-15 | AvatarManager (libloading) | 2 | PR-14 | `📋 TODO` |
| PR-16 | /avatar コマンド | 2 | PR-15 | `📋 TODO` |
